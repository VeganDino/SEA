...
import com.sea.common.model.response.VerifyResult;
...
public class JwtTokenUtil {
	...
	public static String createAccessToken(long userId, String userAddress) {
		...
		return JWT.create().withClaim("userId", userId) // 아이디 저장
	...
	public static String createRefreshToken() {
		...
		return JWT.create().withExpiresAt(expires) // 만료시간
				.withIssuer(ISSUER)
				.withIssuedAt(Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant())) // 토큰 발행 시간 정보
				.sign(Algorithm.HMAC512(secretKey.getBytes())); // 사용할 암호화 알고리즘
	}
	...
	public int getUserId(Cookie[] cookies) {
		for (Cookie cookie : cookies) {
			if ("accessToken".equals(cookie.getName())) {
				return (int)getUserId(cookie.getValue());
			}
		}
